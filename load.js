    ...
    if (headerWrap && headerWrap.innerHTML.trim() === "") {
        fetch("header.html")
            ...
            .then(html => {
                ...
                // Boot the clock immediately after the HTML finishes injecting!
...
function executeSystemTimeTicks() {
    function renderTime() {
        // Query elements dynamically inside the loop in case templates reload
...
(function autoSynchronizeAudioState() {
    ...
    if (cachedSrc && (!window.audioEngine.src || window.audioEngine.src === window.location.href)) {
        ...
        if (localStorage.getItem("audio_was_playing") === "true") {
            window.audioEngine.play().catch((err) => {
                console.log("Audio autoplay prevented on load:", err);
