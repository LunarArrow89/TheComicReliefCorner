...
// Inject Styles on Execution/* ==========================================================================
   LOAD.JS - PART 1: COMPONENT ROUTING ENGINE & INJECTED STYLES
   ========================================================================= */
...
function executeGlobalTemplateLighter() {
    ...
    const headerWrap = document.getElementById("header-container");
    if (sidebarWrap && sidebarWrap.innerHTML.trim() === "") {
        fetch("sidebar.html").then(res => res.text()).then(html => { sidebarWrap.innerHTML = html; });
    }
    if (headerWrap && headerWrap.innerHTML.trim() === "") {
...
function executeSystemTimeTicks() {
    ...
    if (timeBox || dateBox) {
        const now = new Date();
        if (timeBox) timeBox.textContent = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        if (dateBox) dateBox.textContent = now.toLocaleDateString([], { month: 'short', day: 'numeric' }).toUpperCase();
    }
}
...
(function autoSynchronizeAudioState() {
    ...
    if (cachedSrc && (!window.audioEngine.src || window.audioEngine.src === window.location.href)) {
        window.audioEngine.src = cachedSrc;
        if (savedTime) window.audioEngine.currentTime = parseFloat(savedTime);
        if (localStorage.getItem("audio_was_playing") === "true") {
            window.audioEngine.play().catch(() => { localStorage.setItem("audio_was_playing", "false"); });
        }
    }
})();

// Custom Stylesheet Generation Injection
...
if (!document.getElementById(styleId)) {
    ...
    .global-mini-player {
        ...
        bottom: 20px;
        right: 20px;
        width: 350px !important; /* Widened slightly to elegantly make room for the close button */
    ...
    .mini-btn:hover { background: #ff69b4; }
    
    /* Clean, sleek matching close button styling */
    .mini-close-btn {
        background: #444;
        border: none;
        color: #fff;
...
/* ==========================================================================
   LOAD.JS - PART 2: DYNAMIC LAYOUT MARKEUPS & AUDIO SYNCHRONIZATION
   ========================================================================== */
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function processGlobalMiniplayerVisibility() {
        ...
        if (!isMusicTabActive && cachedSrc && !isManuallyHidden) {
            if (!miniPlayer) {
                // FIXED: Aggregated strings updated below to append the ✕ button cleanly into the live layout stream
                ...
                markup += ' <button id="mini-deck-pause-btn" class="mini-btn" title="Play/Pause">Ⅱ</button>';
                markup += ' <button id="mini-deck-close-btn" class="mini-close-btn" title="Close Player">✕</button>';
                markup += '</div>';
                markup += '</div>';
    ...
    function setupMiniPlayerControllers() {
        ...
        // Action binding listener cleanly killing the active tracking node
        if (miniCloseBtn) {
    ...
    function setupMoveableDragEngine(element) {
        ...
        function dragStartInitiation(e) {
            if (e.target.closest(".mini-btn") || e.target.closest(".mini-close-btn") || e.target.closest("#mini-deck-scrub-bar")) return;
            e.preventDefault();
...
/* ==========================================================================
   LOAD.JS - PART 2: EVENT BINDINGS, PROGRESS RENDERS & MOVING MATRIX
   ========================================================================== */
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 4. MOVEABLE UNIVERSAL MINI-PLAYER SYSTEM CONTROLS & EVENT BINDINGS
    function processGlobalMiniplayerVisibility() {
        const isMusicTabActive = document.getElementById("song-search") !== null;
        let miniPlayer = document.getElementById("shared-global-mini-deck");
        const cachedSrc = localStorage.getItem("audio_active_src");
